...
| `modal` | Serverless cloud | `pip install modal` + `modal token new` (opens browser) or `MODAL_TOKEN_ID`/`MODAL_TOKEN_SECRET`; deploy with `modal deploy -m src.sandbox.modal_app --env main --name ts-agents-sandbox`; optional stream/file flags: `TS_AGENTS_MODAL_STREAM`, `TS_AGENTS_MODAL_LOG_FILE` |
