...
`ts-agents` includes a hosted Gradio profile for public deployments such as
Hugging Face Spaces. For installed-package use, run `ts-agents-hosted`. For
source-checkout deployments, the repo still provides the root `app.py` wrapper.
...
2. Upload this repo (or mirror the relevant files) so the Space sees:
   ...
   - `requirements.txt`
   - `ts_agents/`
3. Let Spaces install dependencies from `requirements.txt`. This file is the
   source-checkout deployment manifest for Spaces and should stay aligned with
   the package's runtime dependencies, without dev-only tools such as `pytest`.
...
```bash
TS_AGENTS_ENABLE_AGENT=true OPENAI_API_KEY=your-key ts-agents-hosted
```
