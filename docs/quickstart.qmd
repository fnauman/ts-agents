...
For current low-level plot-producing tools, inspect `result.artifacts[*].path` in the
saved JSON rather than expecting extracted PNGs. `--extract-images` only
applies to saved legacy outputs that still contain `[IMAGE_DATA:...]` tokens.
If you want forecast plots and reports, prefer `workflow run forecast-series
--output-dir ...`, which writes artifact files directly.
